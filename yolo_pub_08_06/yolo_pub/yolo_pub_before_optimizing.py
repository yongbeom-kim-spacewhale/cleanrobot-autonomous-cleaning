...
class YoloViewer(Node):
    ...
    def process_pair(self, rgb_message: Image) -> None:
        ...
        try:
            ...
            # 추론이 오래 걸리더라도 촬영 시각의 TF를 조회하므로, 추론 후에는
            # 현재 /clock과 비교해 영상을 다시 폐기하지 않는다. 입력 직후의 stale 검사는 유지한다.
            if any(points_by_class.values()) and depth_message.header.frame_id != self.target_frame:
                ...
                try:
                    ...
                    if now_wall_ns - self.last_cloud_debug_wall_ns >= 1_000_000_000:
                        for debug_class_id, debug_points in points_by_class.items():
                            if debug_points:
                                center = np.mean(np.asarray(debug_points), axis=0)
