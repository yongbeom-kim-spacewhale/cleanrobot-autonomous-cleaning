...
class YoloViewer(Node):
    ...
    def __init__(self) -> None:
        ...
        # model_path = Path(get_package_share_directory('yolo_pub')) / 'resource' / 'yolo-v8s-best.pt'
        model_path = Path(get_package_share_directory('yolo_pub')) / 'resource' / 'finetune_v8n_best.pt'
        topic, depth_topic, camera_info_topic = '/robot_arm/rgb', '/robot_arm/depth', '/robot_arm/depth/camera_info'
        ...
        cloud_prefix, debug_robot_arm_image_topic, robot_arm_tf_topic = '/robot_arm/trash_points', '/robot_arm/yolo/overlay', '/tf'
        ...
        self.depth_subscription = self.create_subscription(Image, depth_topic, self.depth_callback, qos)
        self.info_subscription = self.create_subscription(CameraInfo, camera_info_topic, self.info_callback, qos)
        ...
        self.amr_depth_subscription = self.create_subscription(Image, amr_depth_topic, self.amr_depth_callback, qos)
        ...
        self.debug_image_pub = self.create_publisher(
            Image, debug_robot_arm_image_topic, QoSProfile(depth=1, reliability=ReliabilityPolicy.RELIABLE)
        )
    ...
    def process_amr_pair(self, rgb_message: Image) -> None:
        ...
        try:
            ...
            # # ================= [추가된 부분] 디버그 Overlay 발행 =================
            # # 구독자(subscriber)가 있을 때만 이미지를 처리하여 추론 부하를 방지합니다.
    ...
    def process_pair(self, rgb_message: Image) -> None:
        ...
        try:
            ...
            # XYZ는 depth pixel + depth CameraInfo로 계산했으므로 depth optical frame에 속한다.
            # 따라서 RGB header가 아니라 depth header를 PointCloud2 frame과 TF source로 사용한다.
            cloud_header = depth_message.header
            if any(points_by_class.values()) and depth_message.header.frame_id != self.target_frame:
                # 점군은 depth camera frame에서 만들어졌으므로 TF로 로봇 base_link 등 기준 프레임으로 옮긴다.
                try:
                    transform = self.tf_buffer.lookup_transform(
                        self.target_frame, depth_message.header.frame_id, Time.from_msg(depth_message.header.stamp)
                    ).transform
