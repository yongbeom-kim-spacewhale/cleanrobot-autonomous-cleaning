...
class YoloViewer(Node):
    ...
    def __init__(self) -> None:
        ...
        self.confidence, self.imgsz = 0.7, 640
        ...
        # RGB-D 입력은 계속 받되, 로봇팔과 AMR 오버레이는 최대 25 Hz로 처리한다.
        # AMR 주행 목표만 5 Hz로 제한해 제어 토픽의 부하·흔들림을 낮춘다.
        self.arm_process_interval_ns = 40_000_000
        self.amr_overlay_interval_ns, self.amr_process_interval_ns = 40_000_000, 200_000_000
        self.last_arm_process_ns: int | None = None
        self.last_amr_overlay_ns: int | None = None
        ...
        # 이 실행본은 can(0), plastic(2)만 사용한다. paper(1), general_waste(3)는
        # 오버레이·AMR 목표·점군 토픽에서 모두 제외한다.
        self.class_ids = (0, 2)
        self.points_pubs = {
    ...
    def sensor_time_is_valid(self, message: Image, label: str) -> bool:
        ...
        # /clock publisher가 없는 standalone/일반 ROS 실행에서는 카메라 timestamp를
        # 검증할 기준이 없다. 이때 프레임을 모두 버리면 AMR·로봇팔 추론이 전혀
        # 실행되지 않으므로, /clock을 실제로 받기 전까지는 입력을 그대로 허용한다.
    ...
    def process_amr_pair(self, rgb_message: Image) -> None:
        ...
        if not self.sensor_time_is_valid(depth_message, '/front_stereo_camera/amr/depth pair'):
            return
        # 오버레이 생성·YOLO 추론은 최대 25 Hz로 별도 제한한다.
        if not self.should_process(rgb_message, 'last_amr_overlay_ns', self.amr_overlay_interval_ns):
            return
        candidate = None
        try:
            ...
            if result.masks is not None and result.boxes is not None:
                ...
                for index, mask in enumerate(result.masks.data.cpu().numpy()):
                    if int(result.boxes.cls[index].item()) not in self.class_ids:
                        continue
                ...
                # result.plot()도 can/plastic만 그리도록 같은 인덱스만 남긴다.
                if keep_indices:
        ...
        # 연속 검출 횟수는 모든 오버레이 프레임에서 갱신한다.
        stable_candidate = self.stable_amr_target(candidate)
        # 오버레이는 매 추론마다 갱신하지만, 주행 제어 메시지는 기존 5 Hz만 발행한다.
