...
2. 로봇팔 영상은 0.70 m 이내의 객체만 클래스별 XYZ PointCloud2로 발행한다.
3. AMR 영상은 화면 중앙에 가까운 후보를 고르고, 3회 연속 검출되어야 주행 목표로 발행한다.
4. 로봇팔 점군은 가능하면 카메라 좌표계에서 base_link 좌표계로 TF 변환한다.

GUI/overlay 이미지는 만들지 않는다. 이 노드는 추론 결과 토픽 발행만 담당한다.
"""
...
class YoloViewer(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter('camera_info_topic', '/robot_arm/camera_info')
        ...
        self.declare_parameter('arm_max_distance_m', 0.70)
        # PointCloud2의 기준 frame. 카메라 frame과 다르면 /tf로 이 frame에 맞춘다.
        self.declare_parameter('target_frame', 'base_link')
        # 전역 /tf를 구독하지 않고 로봇팔 전용 TF만 받아 Carter/AMR TF와 분리한다.
        self.declare_parameter('robot_arm_tf_topic', '/tf/robot_arm')
        ...
        # 두 카메라 각각의 YOLO 추론/결과 발행 최대 주기. 0.2초 = 최대 5 Hz.
        self.declare_parameter('process_interval_sec', 0.2)
        ...
        # 로봇팔은 70 cm 이내의 물체만 접근하므로, 이 밖의 검출은 결과 전체에서 제외한다.
        self.arm_max_distance_m = float(self.get_parameter('arm_max_distance_m').value)
        # AMR은 접근 전 탐색용이므로 로봇팔의 0.70 m 필터와 독립된 범위를 쓴다.
        self.amr_max_distance_m = float(self.get_parameter('amr_max_distance_m').value)
        # RGB-D 입력은 계속 받되, 추론과 결과 토픽은 설정 주기마다 한 번만 처리한다.
        # 따라서 고FPS 카메라를 연결해도 YOLO와 PointCloud2가 불필요하게 과부하되지 않는다.
        self.process_interval_ns = int(float(self.get_parameter('process_interval_sec').value) * 1_000_000_000)
        ...
        # 전역 /tf에는 AMR·LiDAR 등 모든 장치 TF가 섞인다. 로봇팔 전용 topic만 Buffer에 넣는다.
    ...
    def robot_arm_tf_callback(self, message: TFMessage) -> None:
        """/tf/robot_arm의 transform만 TF buffer에 등록한다.
        전역 /tf를 구독하면 Nova Carter의 카메라·LiDAR TF까지 들어와 시간 reset 시
        ``TF_OLD_DATA`` 경고가 발생한다. 이 콜백은 로봇팔 카메라→base_link 변환에
        필요한 transform만 받도록 TF tree를 분리한다.
        """
        for transform in message.transforms:
            self.tf_buffer.set_transform(transform, 'robot_arm_tf')

    def should_process(self, message: Image, last_attribute: str) -> bool:
        """카메라별 추론·출력을 설정한 주기(기본 0.2초)로 제한한다.
        ...
        if previous_ns is not None and current_ns >= previous_ns and current_ns - previous_ns < self.process_interval_ns:
            return False
    ...
    def process_amr_pair(self, rgb_message: Image) -> None:
        ...
        if not self.should_process(rgb_message, 'last_amr_process_ns'):
            return
    ...
    def process_pair(self, rgb_message: Image) -> None:
        ...
        if not self.should_process(rgb_message, 'last_arm_process_ns'):
            return
        try:
            bgr = self.to_bgr(rgb_message)
            ...
            if result.masks is not None and result.boxes is not None:
                ...
                for index, mask in enumerate(masks):
                    ...
                    # 0.70 m 이내 + 그리퍼 ROI 밖인 결과만 화면/점군 처리 대상으로 남긴다.
                    keep.append(
            ...
            if result.masks is not None and result.boxes is not None:
                ...
                for index, mask in enumerate(masks):
                    ...
                    # 로봇팔 필터를 통과한 70 cm 이내의 유효 마스크 depth만 점군으로 만든다.
